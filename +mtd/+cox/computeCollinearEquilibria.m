function eqPoints = computeCollinearEquilibria(massRatio, ax, options)
    ...
    %   acceleration.
    arguments
        massRatio {mtd.crtbp.mustBeAValidMassRatio} % Mass ratio of the system
        ax (1, 1) double % Acceleration in the x direction
        options.tol double {mtd.util.mustBeValidTolerance} = 1e-12 % Newton-Raphson function tolerance
    ...
    for k = 1:length(ax)
        % Fast path for CRTBP (a = 0)
        if ax(k) == 0
            lagrangePoints = crtbpLagrangePoints(massRatio);
            eqPoints(:, :, k) = lagrangePoints(1:3, :);
        else
            eqPoints(:, :, k) = internalSolver(massRatio, ax(k), options);
        end
    end

end

function q = internalSolver(mu, ax, options)
    % Determine gamma values
    [guess1, guess2, guess3] = generateInitialGuesses(mu);
    g1 = simpleNewtonRaphson(@(x) fdf1(mu, ax, x), guess1, options);
    g2 = simpleNewtonRaphson(@(x) fdf2(mu, ax, x), guess2, options);
    g3 = simpleNewtonRaphson(@(x) fdf3(mu, ax, x), guess3, options);
